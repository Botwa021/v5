...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  if (m.quoted) {
    await conn.groupParticipantsUpdate(m.chat, [quoted.sender], "add").catch(_ => _)
  }
  ...
  )).filter(v => v[1]).map(v => v[0] + '@s.whatsapp.net')
  let response = await conn.groupParticipantsUpdate(m.chat, Array.isArray(users) ? users : [users], "add")
  if (response[users] == 409) throw `User has been banned`
